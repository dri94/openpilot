...
class CarInterfaceBase():
  ...
  def create_common_events(self, cs_out, extra_gears=None, gas_resume_speed=-1, pcm_enable=True):
    ...
    if cs_out.gearShifter != GearShifter.drive and cs_out.gearShifter not in extra_gears and not\
       (cs_out.gearShifter == GearShifter.unknown and self.CS.out.gearShifter != GearShifter.unknown):
    ...
    if (self.disengage_on_gas and cs_out.gasPressed and (not self.CS.out.gasPressed) and
       (cs_out.vEgo > gas_resume_speed)) or \
